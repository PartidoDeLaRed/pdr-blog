...
class Wp2CloudPlugin {
    ...
    function addhooks() {
        ...
        add_filter('theme_mod_header_image', array(&$this, 'render_wbid_url'), 9, 1);
        add_filter('theme_mod_background_image', array(&$this, 'render_wbid_url'), 9, 1);
        add_filter('the_content', array(&$this, 'render_wbid_urls'));
    ...
    //--------
    // Gets the direct cloud storage URL for the media content for non-admin
    // pages when rendering them.
    function render_wbid_url($url)
    {
        if (strpos($_SERVER['PHP_SELF'], '/wp-admin/') === false)
    ...
    function xlate_wbid_url($matches)
    {
        ...
        // $matches[1] - the weblob_id, e.g. foo/42

        $weblob_url = $this->get_url_from_weblob_id($matches[1]);
        return $weblob_url ? '"' . $weblob_url .'"' : $matches[0];
    }

    //--------
    // Gets the direct cloud storage URL for the media content when rendering them.
    function render_wbid_urls($content)
    {
        return preg_replace_callback( '#"[^"]+/wp2cloud.php\?weblob_id=([^"]+)"#', array(&$this, 'xlate_wbid_url'), $content);
    }
...
if(strpos($_SERVER['PHP_SELF'], basename(__FILE__)) !== false && !empty($_GET['weblob_id']))
{
    ...
    if($img_url)
        header('Location: ' . $img_url );  // redirect
}
